module.exports = function(graph) {
  ...
  loadingModule.setSuccessful = function() {
    ...
    if (graph.onRenderSuccess) {
      graph.onRenderSuccess();
    }
  };
  ...
  loadingModule.from_IRI_URL = function(fileName) {
    ...
    if (ontologyMenu.cachedOntology(filename)) {
      ...
      requestServerTimeStampForIRI_Converte(ontologyMenu.callbackLoad_Ontology_FromIRI, [
        'convert?iri=' + encoded,
        filename,
      ]);
    }
  ...
  graph.setGraphData = function(data) {
    parseOntologyContent(data);
  };
