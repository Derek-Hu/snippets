...
module.exports = function(graph) {
  ...
  sidebar.clearOntologyInformation = function() {
    ...
    if (isTriggerActive) {
      // close accordion
      d3.select('#selection-details-trigger')
        .node()
        .click();
    }
    showSelectionAdvice();
  ...
  function setLanguages(languages) {
    ...
    var languageSelection = d3.select('#language').on('change', function() {
  ...
  function displayGraphStatistics(deliveredMetrics, statistics) {
    ...
    d3.select('#datatypePropertyCount').text(
      deliveredMetrics.datatypePropertyCount || statistics.datatypePropertyCount()
    );
  ...
  function parseAnnotations(annotationObject) {
  ...
  function listAnnotations(container, annotationObject) {
    // Collect the annotations in an array for simpler processing
    var annotations = parseAnnotations(annotationObject);
  ...
  sidebar.updateSelectionInformation = function(selectedElement) {
    ...
    const isProperty = elementTools.isProperty(selectedElement);
    const isNode = elementTools.isNode(selectedElement);
    graph.onElementSelect && graph.onElementSelect(selectedElement ? selectedElement.drNode : null);
    if (isProperty) {
  ...
  sidebar.updateGeneralOntologyInfo = function() {
    ...
    if (generalMetaObj.hasOwnProperty('description')) {
      if (typeof generalMetaObj.description === 'object') {
        d3.select('#description').node().innerHTML = languageTools.textInLanguage(
