module.exports = function(graph) {
  ...
  graph.setDynamicLabelWidth = function(val) {
    graph.options().maxLabelWidth(val);
  ...
  function addLabelWidthSlider(selector, identifier, label, onChangeFunction) {
    ...
    slider.on('input', function() {
      ...
      if (graph.options().dynamicLabelWidth() === true) graph.animateDynamicLabelWidth();
    });
  ...
  function addCheckBox(identifier, modeName, selector, onChangeFunc, updateLvl) {
    var configOptionContainer = d3
      .select(selector)
      .append('div')
      .classed('checkboxContainer', true);
    var configCheckbox = configOptionContainer
      .append('input')
