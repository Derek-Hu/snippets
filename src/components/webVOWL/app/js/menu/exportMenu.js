...
module.exports = function(graph) {
  ...
  function exportTurtle() {
    ...
    if (success) {
      ...
      exportTurtleButton.attr('href', dataURI).attr('download', ontoTitle + '.ttl');
  ...
  exportMenu.exportAsUrl = function() {
    ...
    currObj.filter_datatypes = String(
      graph
        .options()
        .filterMenu()
        .getCheckBoxValue('datatypeFilterCheckbox')
    );
    currObj.filter_sco = String(
      graph
        .options()
        .filterMenu()
        .getCheckBoxValue('subclassFilterCheckbox')
    );
    currObj.filter_disjoint = String(
      graph
        .options()
        .filterMenu()
        .getCheckBoxValue('disjointFilterCheckbox')
    );
    ...
    currObj.mode_scaling = String(
      graph
        .options()
        .modeMenu()
        .getCheckBoxValue('nodescalingModuleCheckbox')
    );
    ...
    currObj.mode_pnp = String(
      graph
        .options()
        .modeMenu()
        .getCheckBoxValue('pickandpinModuleCheckbox')
    );
    ...
    if (optsString.length === 0) {
      ...
      // remove the all options form window.location
      ...
      if (lPos === -1) {
        ...
        htmlElement.value = String(window.location);
        htmlElement.title = String(window.location);
        return; // nothing to change in the window.location String
      }
  ...
  function exportSvg(isOnlyData) {
    ...
    if (isOnlyData === true) {
      ...
      graph.lazyRefresh();
      return {
        svg: escapedGraphSvgCode,
        dataURI,
      };
    }
    exportSvgButton.attr('href', dataURI).attr('download', exportFilename + '.svg');
  ...
  function inlineVowlStyles() {
    ...
    // setStyleSensitively('.dashed, .anonymous', [{ name: 'stroke-dasharray', value: '2' }]);
    setStyleSensitively('.dotted', [{ name: 'stroke-dasharray', value: '3' }]);
    setStyleSensitively('rect.focused, circle.focused', [{ name: 'stroke-width', value: '2px' }]);
    setStyleSensitively('.nostroke', [{ name: 'stroke', value: 'none' }]);
    setStyleSensitively('marker path', [{ name: 'stroke-dasharray', value: '100' }]);

    const inlineStyles = {
      '.nodeContainer circle.outer': { r: 50 },
      '.nodeContainer circle.middle': { r: 48 },
      '.nodeContainer circle.inner': { r: 48 },
      '.node.noAid circle': {
        fill: '#d5e6ff',
        stroke: '#d5e6ff',
      },
      ...
      '.node.hasRisk circle.inner': {
        'stroke-width': 0,
      },
      '.node.hasRisk circle.middle': {
        'stroke-width': 0,
      },
      '.node.hasRisk circle.outer': {
        fill: '#abccff',
        'stroke-dasharray': 5,
      },
      '.node.hasRisk circle.focused': {
        'stroke-dasharray': 0,
      },

      '.node.overDue circle.outer': {
        stroke: '#9f9f9f',
        r: 50,
        fill: '#9f9f9f',
        'stroke-width': 0,
      },
      '.node.overDue circle.middle': {
        'stroke-width': 0,
        r: 45,
        fill: '#9f9f9f',
      },
      '.node.overDue circle.inner': {
        fill: '#000',
        'stroke-width': 0,
        r: 40,
      },
      '.node.overDue circle.focused': {
        'stroke-width': 2,
      },

      '.node.antiOrg circle.outer': {
        stroke: '#000',
        r: 50,
        fill: '#fff',
      },
      '.node.antiOrg circle.middle': {
        stroke: '#000',
        r: 45,
        fill: '#fff',
      },
      '.node.antiOrg circle.inner': {
        fill: '#838383',
        'stroke-width': 8,
        stroke: '#fff',
        r: 40,
      },
      '.node.antiOrg circle.focused': {
        'stroke-width': 2,
      },

      '.node.antiTeam circle.outer': {
        'stroke-width': 2,
        fill: '#fff',
        r: 50,
      },
      '.node.antiTeam circle.middle': {
        'stroke-width': 0,
        r: 45,
        fill: '#fff',
      },
      '.node.antiTeam circle.inner': {
        fill: '#838383',
        'stroke-width': 0,
        r: 45,
      },
      '.node.antiTeam circle.focused': {
        'stroke-width': 2,
      },
      '.node.overDue tspan': {
        fill: '#fff',
    ...
    Object.keys(inlineStyles).forEach(className => {
      const styles = Object.keys(inlineStyles[className]).reduce((styles, attr) => {
        styles.push({
          name: attr,
          value: inlineStyles[className][attr],
        });
        return styles;
      }, []);
      setStyleSensitively(className, styles);
    });
  }
    ...
    for (i = 0; i < pLen; i++) {
      ...
      if (properties[i].subproperties()) {
        ...
        for (j = 0; j < subProps.length; j++) {
          if (subProps[j].id) subPropsIdArray.push(subProps[j].id());
        }
      ...
      if (properties[i].superproperties()) {
        ...
        for (j = 0; j < superProps.length; j++) {
          if (superProps[j].id) superPropsIdArray.push(superProps[j].id());
        }
      ...
      if (properties[i].inverse()) {
        if (properties[i].inverse().id) pAttr.inverse = properties[i].inverse().id();
      }
    ...
    nodeElements.each(function(node) {
      ...
      for (i = 0; i < classAttribute.length; i++) {
        ...
        if (classObj.id === nodeId) {
          ...
          if (node.pinned()) classObj.pinned = true;
          break;
        }
      }
    });
  ...
  function exportJson() {
    ...
    if (!jsonExportFileName.endsWith('.json')) jsonExportFileName += '.json';
  ...
  function exportTex() {
    ...
    nodeElements.each(function(node) {
      ...
      var qType = 'owlClass';
      if (node.type() === 'owl:Thing' || node.type() === 'owl:Nothing') qType = 'owlThing';
    ...
    for (i = 0; i < propElements.length; i++) {
      ...
      if (correspondingProp.textBlock && correspondingProp.textBlock()) {
        var txtColor = correspondingProp
          .textBlock()
          ._textBlock()
          .style('fill');
      ...
      if (correspondingProp.inverse()) {
        ...
        if (inv_correspondingProp.textBlock && inv_correspondingProp.textBlock()) {
          ...
          var inv_tspans = inv_correspondingProp
            .textBlock()
            ._textBlock()
            .node().children;
    ...
    exportTexButton.attr('href', dataURI).attr('download', exportFilename + '.tex');
  }
