...
module.exports = function(graph) {
  ...
  searchMenu.requestDictionaryUpdate = function() {
    ...
    for (var i = 0; i < numEntries; i++) htmlCollection[0].remove();
  ...
  function userNavigation() {
    ...
    if (newSelection !== selectedEntry) {
      ...
      if (newSelection >= 0 && newSelection < numEntries) {
        ...
        if (selectedEntry >= 0) htmlCollection[selectedEntry].setAttribute('class', 'dbEntry');
      }
    }
  }
  ...
  function createDropDownElements(searchText) {
    var numEntries;
    var copyRes = results;
    var i;
    var token;
    ...
    var lc_text = searchText || searchLineEdit.node().value.toLowerCase();
    ...
    const searchEntries = [];
    console.log('newResultsIds', newResultsIds);
    ...
    for (i = 0; i < numEntries; i++) {
      ...
      // console.log('mergedIdList', mergedIdList);
      ...
      if (croppedText !== newResults[i]) {
        ...
        if (eLen > 1 && allSame === false) {
          if (eLen !== visible) croppedText += '... (' + visible + '/' + eLen + ')';
        } else {
          croppedText += '...';
        }
      ...
      searchEntries.push({
        title: croppedText,
        id: testEntry._id,
        disabled: testEntry.onclick === loop,
      });
  ...
  graph.searchElement = function() {
    if (dictionaryUpdateRequired) {
      return updateSearchDictionary(true);
    }
    // graph.resetSearchHighlight();
    ...
    // if (inputText.length !== 0) {
    //   createSearchEntries();
    //   return createDropDownElements(inputText);
    // }
  };

  graph.locateElementById = function(elementId) {
    var correspondingIds = mergedIdList[elementId];
