...
module.exports = function(graph) {
  ...
  exportTTLModule.requestExport = function() {
    ...
    Map_ID2Node = {};
    Map_ID2Prop = {};
    if (property_success === false || class_success === false) return false;
    return true;
  };
  ...
  function preparePrefixRepresentation() {
    ...
    for (i = 0; i < allNodes.length; i++) {
      ...
      if (prefixModule.validURL(nodeIRI) === true) allNodes[i].prefixRepresentation = '<' + nodeIRI + '>';
      else allNodes[i].prefixRepresentation = nodeIRI;
    }
    for (i = 0; i < allProps.length; i++) {
      var propIRI = prefixModule.getPrefixRepresentationForFullURI(allProps[i].iri());
      if (prefixModule.validURL(propIRI) === true) allProps[i].prefixRepresentation = '<' + propIRI + '>';
      else allProps[i].prefixRepresentation = propIRI;
    }
  }
  ...
  function extractClassDescription(node) {
    ...
    for (var e = 0; e < node.equivalents().length; e++) {
      ...
      if (prefixModule.validURL(eqIRI) === true) eqNode_prefRepresentation = '<' + eqIRI + '>';
    ...
    if (arrayOfNodes.length > 0) {
      ...
      for (var duE = 0; duE < arrayOfNodes.length; duE++) {
        ...
        if (prefixModule.validURL(duIri) === true) duNode_prefRepresentation = '<' + duIri + '>';
    ...
    if (arrayOfUnionNodes.length > 0) {
      ...
      for (var uE = 0; uE < arrayOfUnionNodes.length; uE++) {
        if (arrayOfUnionNodes[uE] && arrayOfUnionNodes[uE].iri()) {
          ...
          if (prefixModule.validURL(uIri) === true) uNode_prefRepresentation = '<' + uIri + '>';
    ...
    for (i = 0; i < allProps.length; i++) {
      ...
      if (allProps[i].domain() === node && allProps[i].type() === 'owl:disjointWith') {
        myProperties.push(allProps[i]);
      }
    }
    for (i = 0; i < myProperties.length; i++) {
      ...
      if (myProperties[i].type() === 'owl:someValuesFrom') {
        ...
        if (myProperties[i].range().type() !== 'owl:Thing') {
          objectDef +=
            indent + '                   owl:someValuesFrom ' + myProperties[i].range().prefixRepresentation + '\r\n';
        }
        objectDef += indent + '                 ];\r\n';
        continue;
      }
      ...
      if (myProperties[i].type() === 'owl:allValuesFrom') {
        ...
        if (myProperties[i].range().type() !== 'owl:Thing') {
          objectDef +=
            indent + '                   owl:allValuesFrom ' + myProperties[i].range().prefixRepresentation + '\r\n';
        }
        objectDef += indent + '                 ];\r\n';
        continue;
      }
  ...
  function prepareHeader() {
    resultingTTLContent += '#################################################################\r\n';
    resultingTTLContent +=
      '###  Generated with the experimental alpha version of the TTL exporter of WebVOWL (version 1.1.3) ' +
  ...
  function getOntologyAuthor(indent) {
    ...
    if (languageElement) {
      if (typeof languageElement !== 'object') {
        if (languageElement.length === 0) return ''; // an empty string
