...
module.exports = function(graph) {
  ...
  graph.updateFilter = function(filters) {
    if (!filters) {
      return;
    }
    Object.keys(filters).forEach(filterId => {
      if (filterId === 'degree') {
        setSliderValue(degreeSlider, filters[filterId]);
        degreeSlider.on('change')();
        return;
      }
      const filterItem = checkboxData.find(checkbox => checkbox.id === filterId);
      if (filterItem) {
        filterItem.checkbox.property('checked', filters[filterId]);
        // Call onclick event handlers programmatically
        filterItem.checkbox.on('click')();
      }
    });
  };
  function addFilterItem(filter, identifier, pluralNameOfFilteredItems, selector) {
    ...
    checkboxData.push({ id: identifier, filter: filter, checkbox: filterCheckbox, defaultState: filter.enabled() });
  ...
  function addNodeDegreeFilter(nodeDegreeFilter, container) {
    ...
    sliderContainer = container.append('div').classed('distanceSliderContainer', true);
    ...
    sliderValueLabel = sliderContainer
      .append('label')
      .classed('value', true)
      .attr('for', 'nodeDegreeDistanceSlider')
      .text(0);
