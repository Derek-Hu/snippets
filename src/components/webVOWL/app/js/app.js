...
module.exports = function() {
  ...
  var app = {},
    ...
    customFilter = webvowl.modules.customFilter(graph),
  ...
  app.initialize = function({ onElementSelect, onElementDbClick, onElementRightClick, onRenderSuccess }) {
    /**
     * START 定制后增加的接口
     */
    graph.onElementSelect = (...paramsSelect) => onElementSelect(...paramsSelect);
    graph.onElementDbClick = (...paramsDl) => onElementDbClick(...paramsDl);
    graph.onElementRightClick = (...paramsDl) => onElementRightClick(...paramsDl);
    graph.onRenderSuccess = (...paramsSuc) => onRenderSuccess(...paramsSuc);
    app.exportSvgData = graph.exportSvgData;
    // app.updateFilter = graph.updateFilter;
    app.setPauseValue = graph.setPauseValue;
    app.searchElement = graph.searchElement;
    app.locateElementById = graph.locateElementById;
    app.locateSearchResult = graph.locateSearchResult;
    app.setGraphData = graph.setGraphData;
    app.hideCompactNotation = function(val) {
      graph.setCompactNotation(val);
    };
    app.centerGraph = graph.centerGraph;
    app.getCustomerFilter = graph.getCustomerFilter;
    app.setCustomerFilter = graph.setCustomerFilter;
    app.updateLayerData = graph.updateLayerData;
    app.updateNodeBgColor = graph.updateNodeBgColor;
    app.updateNodeClassName = graph.updateNodeClassName;
    app.update = graph.update;
    app.setDynamicLabelWidth = graph.setDynamicLabelWidth;
    app.setClassDistance = graph.setClassDistance;
    app.setDataTypeDistance = graph.setDataTypeDistance;
    app.showSlider = function(val) {
      graph
        .options()
        .zoomSlider()
        .showSlider(val);
    ...
    options.filterModules().push(customFilter);
  ...
  function loadOntologyFromText(jsonText, filename, alternativeFilename) {
    ...
    if (window.location.hash.indexOf('#new_ontology') !== -1) {
  ...
  function adjustSize() {
    ...
    var canvasArea = document.getElementById('canvasArea').getBoundingClientRect();
    width = canvasArea.width;
    height = canvasArea.height;

    directInputMod.updateLayout();
    d3.select('#blockGraphInteractions').style('width', width + 'px');
  ...
  function adjustSliderSize() {
    ...
    var height = window.innerHeight - BottomBarHeight;
