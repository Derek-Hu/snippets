...
module.exports = function(graph) {
  ...
  function highlightEditButton(enable, name, fill) {
    ...
    if (enable === false) {
      if (fill) editPath.node().style = 'fill: #fff; stroke : #fff; stroke-width : 1px';
      else editPath.node().style = ' stroke : #fff; stroke-width : 1px';

      editRect.style('cursor', 'auto');
    } else {
      if (fill) editPath.node().style = 'fill: #ff972d; stroke : #ff972d; stroke-width : 1px';
      else editPath.node().style = 'stroke : #ff972d; stroke-width : 1px';
    ...
    for (var name in prefixElements) {
      if (prefixElements.hasOwnProperty(name)) {
        ...
        editButton.selectAll('g').on('mouseover', function() {
          ...
          if (enable === false) {
            if (fill) f_editPath.node().style = 'fill: #fff; stroke : #fff; stroke-width : 1px';
            else f_editPath.node().style = ' stroke : #fff; stroke-width : 1px';

            f_editRect.style('cursor', 'auto');
          } else {
            if (fill) f_editPath.node().style = 'fill: #ff972d; stroke : #ff972d; stroke-width : 1px';
            else f_editPath.node().style = 'stroke : #ff972d; stroke-width : 1px';
        ...
        if (name === 'rdf' || name === 'rdfs' || name === 'xsd' || name === 'dc' || name === 'owl') {
  ...
  function changeIriForElement(element) {
    ...
    if (element.focused()) {
      graph
        .options()
        .focuserModule()
        .handle(element, true); // unfocus
      graph
        .options()
        .focuserModule()
        .handle(element, true); // focus
    }
  ...
  editSidebar.updateSelectionInformation = function(element) {
    if (element === undefined) {
      ...
      for (i = 0; i < numEntries; i++) typeEditorSelection.removeChild(htmlCollection[0]);
  ...
  editSidebar.updateGeneralOntologyInfo = function() {
    ...
    if (generalMetaObj.hasOwnProperty('title')) {
      ...
      } else d3.select('#titleEditor').node().value = generalMetaObj.title;
    }
    ...
    if (generalMetaObj.hasOwnProperty('description')) {
      if (typeof generalMetaObj.description === 'object')
        d3.select('#descriptionEditor').node().value = languageTools.textInLanguage(
          generalMetaObj.description,
          preferredLanguage
        );
      else d3.select('#descriptionEditor').node().value = generalMetaObj.description;
  ...
  editSidebar.updateElementWidth = function() {
    ...
    var title_labelWidth =
      d3
        .select('#titleEditor-label')
        .node()
        .getBoundingClientRect().width + 20;
    var iri_labelWidth =
      d3
        .select('#iriEditor-label')
        .node()
        .getBoundingClientRect().width + 20;
    var version_labelWidth =
      d3
        .select('#versionEditor-label')
        .node()
        .getBoundingClientRect().width + 20;
    var author_labelWidth =
      d3
        .select('#authorsEditor-label')
        .node()
        .getBoundingClientRect().width + 20;
    //find max width;
    var maxW = 0;
    ...
    var elementIri_width =
      d3
        .select('#element_iriEditor-label')
        .node()
        .getBoundingClientRect().width + 20;
    var elementLabel_width =
      d3
        .select('#element_labelEditor-label')
        .node()
        .getBoundingClientRect().width + 20;
    var elementType_width =
      d3
        .select('#typeEditor-label')
        .node()
        .getBoundingClientRect().width + 20;
    var elementDType_width =
      d3
  ...
  function addElementsCharacteristics(element) {
    ...
    var filterContainer, filterCheckbox;
    if (elementTools.isNode(element) === true) {
  ...
  function getElementPrototypes(selectedElement) {
    ...
    if (elementTools.isProperty(selectedElement)) {
      if (selectedElement.type() === 'owl:DatatypeProperty') availiblePrototypes.push('owl:DatatypeProperty');
      else {
