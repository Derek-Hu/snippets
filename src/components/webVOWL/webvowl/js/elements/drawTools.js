 ...
 */
const correctSvg = require('~/assets/correct.svg');
const wrongSvg = require('~/assets/wrong.svg');
...
module.exports = (function() {
  ...
  tools.appendCircularClass = function(parent, radius, cssClasses, tooltip, backgroundColor) {
    var circle = parent.append('circle').classed('class outer', true);
    // .attr('r', radius);

    parent.append('circle').classed('class middle', true);
    // .attr('r', radius)

    parent.append('circle').classed('class inner', true);

    parent.append('circle').classed('class layer', true);

    parent
      .append('image')
      .attr('href', correctSvg)
      .attr('width', 32)
      .classed('blue', true);
    parent
      .append('image')
  ...
  tools.drawRectHalo = function(node, width, height, offset) {
    var container;
    if (node.nodeElement) container = node.nodeElement();
    else container = node.labelElement();
    ...
    var haloGroupElement = container.append('g').classed('hidden-in-export', true);
