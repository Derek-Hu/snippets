...
module.exports = (function() {
  var o = function(graph) {
    ...
    this.layerBlock = function(p) {
      if (!arguments.length) return layerBlock;
      layerBlock = p;
      return this;
    };
    ...
    this.draw = function(parentElement, additionalCssClasses) {
      ...
      if (rectangularRepresentation === true) {
        ...
        // 逾期账户、欺诈团伙、欺诈中介 边框宽度设置
        let borderWidth = 0;
        if (['overDue', 'antiTeam', 'antiOrg'].indexOf(this.drType) !== -1) {
          borderWidth = 5;
        }
        renderingElement = drawTools.appendCircularClass(
          parentElement,
          that.actualRadius() - borderWidth,
          cssClasses,
          that.labelForCurrentLanguage(),
          bgColor
        );
    ...
    this.postDrawActions = function() {
      ...
      that.layerBlock(createLayerText());
    ...
    this.redrawLabelText = function() {
      ...
      that.layerBlock().remove();
      that.textBlock(createTextBlock());
      that.layerBlock(createLayerText());
      renderingElement.select('title').text(that.labelForCurrentLanguage());
    };
    function createLayerText() {
      var textBlock = new CenteringTextElement(that.nodeElement(), 'red', 'layerLabel');

      textBlock.addText(' ');
      if (that.drNode && that.drNode.layer && that.drNode.layer.label) {
        textBlock.addSubText(that.drNode.layer.label);
      }
    ...
    function createTextBlock() {
      var bgColor = that.backgroundColor();
      if (that.attributes().indexOf('deprecated') > -1) bgColor = undefined;
      ...
      if (that.drNode && that.drNode.subTitle) {
        textBlock.addSubText(that.drNode.subTitle);
      }
