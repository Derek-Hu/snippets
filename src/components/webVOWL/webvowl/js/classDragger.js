module.exports = function(graph) {
  ...
  Class_dragger.svgRoot = function(root) {
    if (!arguments.length) return Class_dragger.rootElement;
  ...
  Class_dragger.drawNode = function() {
    Class_dragger.pathElement = Class_dragger.pathLayer.append('line').classed('classNodeDragPath', true);
  ...
  Class_dragger.updateElement = function() {
    ...
    var angle =
      (Math.atan2(Class_dragger.parent.y - Class_dragger.y, Class_dragger.parent.x - Class_dragger.x) * 180) / Math.PI;

    Class_dragger.nodeElement.attr(
      'transform',
      'translate(' + Class_dragger.x + ',' + Class_dragger.y + ')rotate(' + angle + ')'
    );
  ...
  Class_dragger.onMouseOut = function() {
    if (Class_dragger.mouseButtonPressed === true) return;
