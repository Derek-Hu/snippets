...
module.exports = function(graph) {
  ...
  parser.parseSettings = function() {
    ...
    if (settingsData.gravity) {
      ...
      graph
        .options()
        .gravityMenu()
        .reset(); // reads the options values and sets the gui values
    }
    ...
    if (settingsData.filter) {
      ...
      if (settingsData.filter.degreeSliderValue) {
        ...
        graph
          .options()
          .filterMenu()
          .setDegreeSliderValue(degreeSliderValue);
      }
      graph
        .options()
        .filterMenu()
    ...
    if (baseObjects) {
      baseObjects.forEach(function(element) {
        ...
        if (Prototype) {
          ...
          var node = new Prototype(graph, element);
          node.drType = element.drType;
          node.drNode = element;
          node
    ...
    if (baseObjects) {
      baseObjects.forEach(function(element) {
        ...
        if (Prototype) {
          ...
          property.drType = element.drType;
          property
            .annotations(element.annotations)
            .baseIri(element.baseIri)
  ...
  function createPropertyStructure(rawProperties, classMap, propertyMap) {
    ...
    // Connect properties
    rawProperties.forEach(function(property) {
      var domain, range, domainObject, rangeObject, inverse;
  ...
  function getOtherEqualProperty(properties, referenceProperty) {
    ...
    for (i = 0, l = properties.length; i < l; i++) {
      ...
      if (referenceProperty.domain() !== property.domain() || referenceProperty.range() !== property.range()) {
        continue;
      }
