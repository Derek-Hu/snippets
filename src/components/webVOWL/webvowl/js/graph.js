...
function defaultCustomerFilter() {
  return true;
}
...
module.exports = function(graphContainerSelector) {
  var graph = {},
    ...
    zoom,
    customerFilter = defaultCustomerFilter;
  ...
  graph.getCustomerFilter = function() {
    return customerFilter;
  };
  graph.setCustomerFilter = function(filter) {
    if (!filter) {
      customerFilter = defaultCustomerFilter;
      return;
    }
  ...
  graph.updateZoomSliderValueFromOutside = function() {
    graph
      .options()
      .zoomSlider()
      .updateZoomSliderValue(zoomFactor);
  };
  ...
  graph.setDefaultZoom = function(val) {
    ...
    graph
      .options()
      .zoomSlider()
      .updateZoomSliderValue(defaultZoom);
  };
  ...
  function hiddenRecalculatePositions() {
    ...
    if (
      graph
        .options()
        .loadingModule()
        .successfullyLoadedOntology() === false
    ) {
      force.stop();
    ...
    if (updateRenderingDuringSimulation === false) {
      ...
      if (value > 0.49) {
        ...
        if (initialLoad) {
          if (graph.paused() === false) force.resume(); // resume force
          initialLoad = false;
        }
        ...
        if (centerGraphViewOnLoad === true && force.nodes().length > 0) {
          if (force.nodes().length < 10) graph.forceRelocationEvent(true);
          // uses dynamic zoomer;
  ...
  function recalculatePositionsWithFPS() {
    ...
    debugContainer.node().innerHTML =
      'FPS: ' + fps + '<br>Nodes: ' + force.nodes().length + '<br>Links: ' + force.links().length;
    then = Date.now();
  }
  ...
  function addClickEvents() {
    ...
    nodeElements.on('contextmenu', function(clickedNode) {
      ...
      if (graph.onElementRightClick) {
        graph.onElementRightClick(clickedNode && clickedNode.drNode ? clickedNode.drNode : null, {
          x: d3.event.clientX,
          y: d3.event.clientY,
        });
      }
    ...
    nodeElements.on('dblclick', function(clickedNode) {
      if (graph.onElementDbClick) {
        graph.onElementDbClick(clickedNode && clickedNode.drNode ? clickedNode.drNode : null);
      }
      d3.event.stopPropagation();
      if (editMode === true) {
        clickedNode.raiseDoubleClickEdit(defaultIriValue(clickedNode));
  ...
  function redrawContent() {
    ...
    nodeElements = nodeContainer
      ...
      .attr('class', function(node) {
        const customCls = node.drNode.layer && node.drNode.layer.type ? ' layer ' + node.drNode.layer.type : '';
        // console.log(node);
        return (
          'node ' + (node.drNode && node.drNode.className ? node.drNode.className + ' ' : '') + node.drType + customCls
        );
      })
      .attr('id', function(d) {
        return d.id();
  ...
  const updateNodeData = function(id) {
    return unfilteredData.nodes.find(function(node) {
      return node.drNode.id === id;
    });
  };
  graph.updateLayerData = function(id, value) {
    const targetNode = updateNodeData(id);
    targetNode.drNode.layer = value;
    graph.update();
  };

  graph.updateNodeBgColor = function(id, value) {
    const targetNode = updateNodeData(id);
    const maps = {
      white: 'WhiteEntity',
      black: 'BlackEntity',
      gray: 'HasSuspectEntity',
    };
    if (maps[value]) {
      targetNode.drType = maps[value][0].toLowerCase() + maps[value].substring(1);
      graph.update();
    }
  };

  graph.updateNodeClassName = function(id, value) {
    const targetNode = updateNodeData(id);
    if (value) {
      targetNode.drNode.className = value;
      graph.update();
    }
  };
  ...
  graph.clearGraphData = function() {
    ...
    if (sidebar) sidebar.clearOntologyInformation();
    if (graphContainer) redrawGraph();
  };
  ...
  function generateDictionary(data) {
    ...
    for (i = 0; i < nodes.length; i++) {
      ...
      if (nodes[i].labelForCurrentLanguage() !== undefined) originalDictionary.push(nodes[i]);
    }
    var props = data.properties;
    for (i = 0; i < props.length; i++) {
      if (props[i].labelForCurrentLanguage() !== undefined) originalDictionary.push(props[i]);
    }
    ...
    for (i = 0; i < originalDict.length; i++) {
      ...
      var dictElementId;
      if (dictElement.property) dictElementId = dictElement.property().id();
      else dictElementId = dictElement.id();
  ...
  function loadGraphData(init) {
    ...
    if (graphContainer && validOntology === true) {
      ...
      graph
        .options()
        .ontologyMenu()
        .append_bulletPoint(intl.formatMessage({ id: 'generating-association-map' }));
      loadingModule.setPercentMode();
              ...
              if (name === searchString) computeDistanceToCenter(node);
                ...
                if (node.property().equivalents()) {
                  ...
                  for (var e = 0; e < eq.length; e++) {
                    if (!eq[e].getHalos()) eq[e].drawHalo();
                  }
                  if (!node.property().getHalos()) node.property().drawHalo();
  ...
  function computeDistanceToCenter(node, inverse) {
    ...
    if (x < 0 || x > w || y < 0 || y > h) {
      ...
      var dx = wX - node.x;
      var dy = wY - node.y;
      if (highlightOfInv === true) dy = wY - node.y - 20;

      if (highlightOfInv === false && node.property && node.property().inverse()) dy = wY - node.y + 20;
  ...
  graph.locateSearchResult = function() {
    if (pulseNodeIds && pulseNodeIds.length > 0) {
      ...
      if (elementTools.isNode(node)) {
        graph
          .options()
          .focuserModule()
          .handle(node, true);

        graph.onElementSelect && graph.onElementSelect(node ? node.drNode : null);
      }
    }
  };
  ...
  graph.changeNodeType = function(element) {
    ...
    if (typeString === 'rdfs:Datatype') {
      if (aNode.dType() === 'undefined') aNode.label('undefined');
      else {
    ...
    if (remId !== -1) unfilteredData.nodes.splice(remId, 1);
    remId = classNodes.indexOf(element);
    if (remId !== -1) classNodes.splice(remId, 1);
  ...
  graph.changePropertyType = function(element) {
    ...
    if (properties.indexOf(aProp) === -1) properties.push(aProp);
    var remId = unfilteredData.properties.indexOf(element);
    if (remId !== -1) unfilteredData.properties.splice(remId, 1);
    if (properties.indexOf(aProp) === -1) properties.push(aProp);
    remId = properties.indexOf(element);
    if (remId !== -1) properties.splice(remId, 1);
    graph.fastUpdate();
  ...
  function removeEditElements() {
    ...
    if (addDataPropertyGroupElement) addDataPropertyGroupElement.classed('hidden', true);
    if (deleteGroupElement) deleteGroupElement.classed('hidden', true);
  ...
  function addNewNodeElement(element) {
    ...
    if (classNodes.indexOf(element) === -1) classNodes.push(element);
  ...
  graph.getTargetNode = function(position) {
    ...
    if (hoveredNodeElement) {
      ...
      if (minDist > tN.actualRadius() + 30) return null;
      else return tN;
    }
  };
  ...
  graph.genericPropertySanityCheck = function(domain, range, typeString, header, action) {
    if (domain === range && typeString === 'rdfs:subClassOf') {
      graph
        .options()
        .warningModule()
        .showWarning(header, 'rdfs:subClassOf can not be created as loops (domain == range)', action, 1, false);
      return false;
    }
    if (domain === range && typeString === 'owl:disjointWith') {
      graph
        .options()
        .warningModule()
        .showWarning(header, 'owl:disjointWith  can not be created as loops (domain == range)', action, 1, false);
      return false;
    }
    ...
    if (domain.type() === 'owl:Thing' && typeString === 'owl:allValuesFrom') {
      graph
        .options()
        .warningModule()
        .showWarning(header, 'owl:allValuesFrom can not originate from owl:Thing', action, 1, false);
      return false;
    }
    if (domain.type() === 'owl:Thing' && typeString === 'owl:someValuesFrom') {
      graph
        .options()
        .warningModule()
        .showWarning(header, 'owl:someValuesFrom can not originate from owl:Thing', action, 1, false);
      return false;
    }
    ...
    if (range.type() === 'owl:Thing' && typeString === 'owl:allValuesFrom') {
      graph
        .options()
        .warningModule()
        .showWarning(header, 'owl:allValuesFrom can not be connected to owl:Thing', action, 1, false);
      return false;
    }
    if (range.type() === 'owl:Thing' && typeString === 'owl:someValuesFrom') {
      graph
        .options()
        .warningModule()
        .showWarning(header, 'owl:someValuesFrom can not be connected to owl:Thing', action, 1, false);
      return false;
    }
  ...
  graph.checkIfIriClassAlreadyExist = function(url) {
    ...
    var allNodes = unfilteredData.nodes;
    for (var i = 0; i < allNodes.length; i++) {
      if (elementTools.isDatatype(allNodes[i]) === true || allNodes[i].type() === 'owl:Thing') continue;
      ...
      for (i = 0; i < allProps.length; i++) {
        ...
        if (
          ...
          graph
            .options()
            .warningModule()
            .showWarning('Warning', 'This triple already exist!', 'Element not created!', 1, false);
          return false;
        }
        if (
          ...
          graph
            .options()
            .warningModule()
            .showWarning('Warning', 'Inverse assignment already exist! ', 'Element not created!', 1, false);
          return false;
        }
      }
      return true;
  ...
  graph.sanityCheckProperty = function(domain, range, typeString) {
    ...
    if (domain.type() === 'owl:Thing' && typeString === 'owl:allValuesFrom') {
      graph
        .options()
        .warningModule()
        .showWarning('Warning', 'owl:allValuesFrom can not originate from owl:Thing', 'Element not created!', 1, false);
      return false;
    }
  ...
  graph.createDataTypeProperty = function(node) {
    ...
    if (classNodes.indexOf(aNode) === -1) classNodes.push(aNode);
  ...
  graph.removeNodeViaEditor = function(node) {
    ...
    for (var i = 0; i < unfilteredData.properties.length; i++) {
      if (unfilteredData.properties[i].domain() === node || unfilteredData.properties[i].range() === node) {
        ...
        if (
          unfilteredData.properties[i].type().toLocaleLowerCase() === 'owl:datatypeproperty' &&
    ...
    if (removedItems > 2) {
      ...
      if (datatypes !== 0) {
        text =
          'You are about to delete 1 class, ' + datatypes + ' datatypes  and ' + propsToRemove.length + ' properties';
      }
  ...
  graph.removePropertyViaEditor = function(property) {
    ...
    if (property.type().toLocaleLowerCase() === 'owl:datatypeproperty') {
      ...
      if (remId !== -1) unfilteredData.nodes.splice(remId, 1);
      if (remId !== -1) remId = classNodes.indexOf(property.range());
  ...
  function doubletap() {
    ...
    var numTouchers = 1;
    if (d3.event && d3.event.touches && d3.event.touches.length) numTouchers = d3.event.touches.length;
  ...
  function touchzoomed() {
    ...
    if (touch_time - last_touch_time < 300 && d3.event.touches.length === 1) {
      ...
      if (editMode === true) {
        ...
        forceNotZooming = false;
        if (originalD3_touchZoomFunction) originalD3_touchZoomFunction();
      }
      return;
    }
    forceNotZooming = false;
  ...
  function setDeleteHoverElementPositionProperty(property, inversed) {
    if (property && property.labelElement()) {
      ...
      if (property.labelElement().attr('transform') === 'translate(0,15)') delY += 15;
  ...
  graph.activateHoverElements = function(val, node, touchBehaviour) {
    ...
    if (val === true) {
      ...
      addDataPropertyGroupElement.classed('hidden', true);
      classDragger.nodeElement.on('mouseover', editElementHoverOn).on('mouseout', editElementHoverOut);
      classDragger.draggerObject.on('mouseover', editElementHoverOnHidden).on('mouseout', editElementHoverOutHidden);
